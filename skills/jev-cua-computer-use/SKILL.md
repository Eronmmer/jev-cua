...
Known live limitation: Cua Driver 0.28.2 currently returns Calculator's buttons but not its display/result in the Accessibility snapshot. Do not attempt or claim a verified multi-button calculation through native mode: there is no deterministic semantic postcondition for the intermediate operator or final result. Stop immediately and report this driver limitation rather than substituting shell automation, ordinary computer use, or receipt-only success.
